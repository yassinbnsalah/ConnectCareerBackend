...
async function getInterviewByRecruiter(userID) {
  try {
    const interviews = await Interview.find({ recruiter: userID }).populate(
      "inviter"
    );
    return interviews;
  } catch (error) {
...
async function createInterview(req, res) {
  try {
    ...
    const recruiterRealName = await User.findById(recruiter).populate(
      "entreprise"
    );
